...
## 行動原則
1. **Think Before Coding** - 不明点は実装前に確認。仮定を勝手に置かない
2. **Simplicity First** - 聞かれたことだけ実装。過剰な抽象化禁止
3. **Surgical Changes** - 依頼に関係ない箇所を触らない。既存コメントやコードを勝手に消さない
4. **Goal-Driven Execution** - 成功基準を先に定義し、テストで検証してから完了とする
