  ...
  return (
    <header className="bg-white border-b border-[#e0e0e0] sticky top-0 z-50">
      <div className="max-w-[1120px] mx-auto px-5 lg:px-8 h-16 flex items-center justify-between">
        <Link
          href="/"
          className="text-[#1a1a1a] font-bold text-base tracking-wide leading-tight hover:opacity-70 transition-opacity"
        ...
        <nav className="flex items-center gap-1 nav-label">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`px-4 py-2 text-[13px] font-medium text-[#5c5c5c] border-b-2 border-transparent transition-colors ${item.color} ${item.border} min-h-[48px] flex items-center`}
            >
              {item.label}
            </Link>
          ))}
